  ...
  <xsl:template match="/">
		<xsl:for-each select="//stuff">
		    <xsl:if test="./stuff_id = $stuff_id">
			    <p class="p_detail">         
					<b>Share-Info ID: </b>  <xsl:value-of select="stuff_id" />  <br />
			    </p>    
			    <p class="p_detail" style="font-family: calibri;font-size:25px ;padding-bottom:10px;margin-bottom:10px">  <b>Title of Post: </b> 
					<input runat="server" type="text"  style="border-radius:10px;width:90%;min-height:25px;margin:10px;padding:5px;font-size:13px;" id="txt_item_title">
						<xsl:attribute name="value">
							 <xsl:value-of select="stuff_title" /> 
						</xsl:attribute>
					</input><br />
					...
					<asp:FileUpload CssClass="btn-success" style="width: 95%;padding: 10px;margin-left:5px;margin-right:20px;font-size: 15px;border-radius: 20px;border: 5px double;background-color: chocolate;opacity: .9;" ID="FileUpload1" ClientIDMode="Static" runat="server" />  
			    ...
			    <p class="p_detail">      
				   <b>Shared by: </b>  <span>
					...
					<xsl:value-of select="stuff_uid" /></span><br />
				</p>
				<p class="p_detail">      
